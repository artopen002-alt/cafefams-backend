...
// একটা model rate-limited হলে পরের গুলো try হবে — সবগুলো FREE
...
═══ সবচেয়ে গুরুত্বপূর্ণ নিয়ম (অবশ্যই মানতে হবে) ═══
যখনই একটা order confirm করবে (উপরের format-এ summary দেওয়ার সময়), reply-এর একদম শেষে — নতুন লাইনে — এই EXACT format-এ একটা data tag যুক্ত করবে:

[ORDER_DATA]{"confirmed":true,"subtotal":<NUMBER>}[/ORDER_DATA]

এখানে <NUMBER> = সব item-এর দামের যোগফল (GST ছাড়া), শুধু সংখ্যা, কোনো ₹ চিহ্ন বা কমা ছাড়া। যেমন: [ORDER_DATA]{"confirmed":true,"subtotal":350}[/ORDER_DATA]

এই tag টা শুধু তখনই লিখবে যখন order সত্যিই confirm হচ্ছে। সাধারণ কথাবার্তা, menu দেখানো, প্রশ্নের উত্তর — এসবে এই tag লিখবে না। Customer-কে এই tag-এর ব্যাপারে কিছু বলবে না বা explain করবে না — এটা শুধু system-এর জন্য।`;
...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    ...
    // একটার পর একটা free model try করো — কোনো একটা না rate-limited হলে সেটাই use হবে
    for (const model of FALLBACK_MODELS) {
      try {
        ...
        if (!response.ok) {
          ...
          continue; // পরের model try করো
        }

        data = await response.json();
        console.log(`✅ Used model: ${model}`);
        break; // success — loop থামাও
      } catch (e) {
        lastErr = e.message;
        continue;
      }
    ...
    if (!data) {
      return res.status(500).json({ error: 'All AI models busy, try again in a moment', detail: lastErr });
    }

    let reply = data.choices?.[0]?.message?.content || 'দুঃখিত, একটু পরে আবার চেষ্টা করুন।';

    // ── ORDER_DATA tag খুঁজো (language-independent detection) ──
    const tagMatch = reply.match(/\[ORDER_DATA\]\s*(\{.*?\})\s*\[\/ORDER_DATA\]/s);
    let orderInfo = null;
    ...
    if (tagMatch) {
      try {
        ...
        console.error('ORDER_DATA parse failed:', e.message);
      }
      // Customer-কে raw tag দেখাবো না
      reply = reply.replace(tagMatch[0], '').trim();
    }

    if (orderInfo) {
      const subtotal = Math.round(orderInfo.subtotal);
      ...
      await sendTelegram(
        ...
        `🪑 Table: ${tableNumber}\n` +
        `💬 Customer: ${message}\n` +
        `💰 Subtotal: ₹${subtotal} + GST ₹${gst} = ₹${total}\n` +
        `⏰ Time: ${new Date().toLocaleTimeString('bn-BD')}`
      );

      // AI যদি নিজের মতো একটা "CF-XXXX" বা "CF-1234" placeholder লিখে থাকে, সেটা real ID দিয়ে replace করো
      reply = reply.replace(/CF-[A-Za-z0-9]{4}/g, orderId);
      if (!reply.includes(orderId)) {
        reply += `\n\nOrder ID: ${orderId}`;
      }

      return res.json({ success: true, reply, orderId, orderSubtotal: subtotal, orderGst: gst, orderTotal: total });
    }
