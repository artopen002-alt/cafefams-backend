...
app.post('/api/chat', async (req, res) => {
  ...
  if (!GEMINI_API_KEY) return res.status(500).json({ error: 'Gemini API key missing in Railway variables' });

  try {
    const geminiMessages = [
      ...history.map(h => ({ role: h.role, parts: [{ text: h.content }] })),
      { role: 'user', parts: [{ text: `Table ${tableNumber}: ${message}` }] }
    ];
    ...
    const reply = data.candidates?.[0]?.content?.parts?.[0]?.text || 'দুঃখিত, একটু পরে আবার চেষ্টা করুন।';
