...
// FIX: restaurant_id was missing from both the DB WHERE clause and the
// Telegram notification — a staff member for one restaurant could update
// (and get notified about) another restaurant's order if they somehow knew
// its orderId. restaurantId now scopes the memory check + DB update the
// same way /api/admin/discount already does. The "Order Ready" alert now
// goes through sendTelegramForRestaurant(req.restaurant) — req.restaurant
// is the same resolved restaurant requireAdmin already authenticated
// against, so no extra resolver lookup is needed — landing on the right
// restaurant's own bot instead of always the global one. Byte-identical for
// 'cafefams' since its resolved bot credentials are unchanged (see
// sendTelegramForRestaurant's own comment).
app.post('/api/admin/order-status', requireAdmin, async (req, res) => {
  const { orderId, kitchenStatus, restaurantId = 'cafefams' } = req.body;
  ...
  const memOrder = (orders[orderId] && (orders[orderId].restaurantId || 'cafefams') === restaurantId)
    ? orders[orderId] : null;

  // Memory update (if present in current session, and belongs to this restaurant)
  if (memOrder) memOrder.kitchenStatus = kitchenStatus;

  // Database update — always runs, even if order not in memory (e.g. after server restart)
  const dbUpdate = await dbQuery(
    `UPDATE orders SET kitchen_status=$1 WHERE order_id=$2 AND restaurant_id=$3 RETURNING session_id, table_number`,
    [kitchenStatus, orderId, restaurantId]
  );

  // If not in memory (for this restaurant) AND not in DB (for this restaurant) — order truly doesn't exist
  if (!memOrder && (!dbUpdate || dbUpdate.rowCount === 0)) {
    return res.status(404).json({ error: 'Order not found' });
  }

  // Real-time push to customer — check memory first, then DB result
  const orderSessionId = (memOrder && memOrder.sessionId)
  ...
  if (kitchenStatus === 'ready') {
    const tableNum = (memOrder && memOrder.tableNumber)
    ...
    await sendTelegramForRestaurant(
      req.restaurant,
...
// FIX: restaurant_id was missing from both the in-memory lookup and the DB
// UPDATE. assist-request ids are just 'AR-' + Date.now(), so two different
// restaurants could even collide on the exact same id under concurrent
// traffic — this scoping isn't just a security nicety, it's a correctness
// fix once a second restaurant is actually live. Same pattern as
// /api/admin/order-status above.
app.post('/api/admin/assist-resolve', requireAdmin, async (req, res) => {
  const { id, restaurantId = 'cafefams' } = req.body;
  if (!id) return res.status(400).json({ error: 'id required' });

  const req_ = assistRequests.find(r => r.id === id && (r.restaurantId || 'cafefams') === restaurantId);
  ...
  const dbUpdate = await dbQuery(
    `UPDATE assist_requests SET resolved=true WHERE id=$1 AND restaurant_id=$2 RETURNING id`,
    [id, restaurantId]
  );
